...
def main():
    ...
    parser.add_argument('-i', '--iteration', help='Iteration', required=False)
    ...
    print('Iteration: {iter}'.format(iter=args.iteration))
    print('\n#######################################################################\n')
    ...
    g2v.save_vectors(args.edgelist.split('.')[0] + '_node2vec_Embeddings' + str(args.dim) + "_" + str(args.iteration) + '.emb')

    #if args.save_model:
        # save node2vec model -- uses a lot of memory and takes a very long time to run on large graphs
        #g2v.save(args.edgelist.split('.')[0] + '_node2vec_Model' + str(args.dim) + '.pkl')
        #g2v = Word2Vec.load(args.edgelist.split('.')[0] + '_node2vec_Model' + str(args.dim) + '.pkl.zip')
